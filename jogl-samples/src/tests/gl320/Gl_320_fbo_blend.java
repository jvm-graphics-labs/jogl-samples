...
public class Gl_320_fbo_blend extends Test {
    ...
    private int[] program = new int[Program.MAX.ordinal()], uniformDiffuse = new int[Program.MAX.ordinal()],
    ...
    private boolean initProgram(GL3 gl3) {
        ...
        if (validated) {
            ...
            program[Program.TEXTURE.ordinal()] = shaderProgram.program();

            gl3.glBindAttribLocation(program[Program.TEXTURE.ordinal()], Semantic.Attr.POSITION, "Position");
            gl3.glBindAttribLocation(program[Program.TEXTURE.ordinal()], Semantic.Attr.COLOR, "Texcoord");
            gl3.glBindFragDataLocation(program[Program.TEXTURE.ordinal()], Semantic.Frag.COLOR, "Color");
        ...
        if (validated) {
            ...
            uniformTransform = gl3.glGetUniformBlockIndex(program[Program.TEXTURE.ordinal()], "transform");
            uniformDiffuse[Program.TEXTURE.ordinal()]
                    = gl3.glGetUniformLocation(program[Program.TEXTURE.ordinal()], "Diffuse");
            uniformDiffuse[Program.SPLASH.ordinal()]
                    = gl3.glGetUniformLocation(program[Program.SPLASH.ordinal()], "Diffuse");

            gl3.glUseProgram(program[Program.TEXTURE.ordinal()]);
            gl3.glUniform1i(uniformDiffuse[Program.TEXTURE.ordinal()], 0);
            gl3.glUniformBlockBinding(program[Program.TEXTURE.ordinal()], uniformTransform, Semantic.Uniform.TRANSFORM0);
