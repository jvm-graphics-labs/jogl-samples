...
public class Gl_320_fbo_blend_points extends Test {
    ...
    private float[] vertexData = {
        -1.0f * scale, -1.0f * scale, 0.0f, 1.0f, 1.0f, 0.5f, 0.0f, 1.0f,
        +1.0f * scale, -1.0f * scale, 0.0f, 1.0f, 0.0f, 1.0f, 0.5f, 1.0f,
        +1.0f * scale, +1.0f * scale, 0.0f, 1.0f, 0.5f, 0.0f, 1.0f, 1.0f,
        -1.0f * scale, +1.0f * scale, 0.0f, 1.0f, 1.0f, 0.0f, 0.5f, 1.0f,
        +1.0f * scale, +0.0f * scale, 0.0f, 1.0f, 0.5f, 1.0f, 0.0f, 1.0f,
        +0.0f * scale, +1.0f * scale, 0.0f, 1.0f, 0.0f, 0.5f, 1.0f, 1.0f,
        -1.0f * scale, +0.0f * scale, 0.0f, 1.0f, 0.4f, 0.6f, 0.5f, 1.0f,
        +0.0f * scale, -1.0f * scale, 0.0f, 1.0f, 0.5f, 0.4f, 0.6f, 1.0f
    };
    ...
    private int[] program = new int[Program.MAX.ordinal()], bufferName = new int[Buffer.MAX.ordinal()],
    ...
    private boolean initProgram(GL3 gl3) {
        ...
        if (validated) {
            ...
            program[Program.RENDER.ordinal()] = shaderProgram.program();

            gl3.glBindAttribLocation(program[Program.RENDER.ordinal()], Semantic.Attr.POSITION, "Position");
            gl3.glBindAttribLocation(program[Program.RENDER.ordinal()], Semantic.Attr.COLOR, "Color");
            gl3.glBindFragDataLocation(program[Program.RENDER.ordinal()], Semantic.Frag.COLOR, "Color");
        ...
        if (validated) {
            ...
            program[Program.SPLASH.ordinal()] = shaderProgram.program();

            gl3.glBindFragDataLocation(program[Program.SPLASH.ordinal()], Semantic.Frag.COLOR, "Color");

            shaderProgram.link(gl3, System.out);
        }
        if (validated) {

            uniformTransform = gl3.glGetUniformBlockIndex(program[Program.RENDER.ordinal()], "transform");
            uniformDiffuse = gl3.glGetUniformLocation(program[Program.SPLASH.ordinal()], "Diffuse");

            gl3.glUseProgram(program[Program.RENDER.ordinal()]);
            gl3.glUniformBlockBinding(program[Program.RENDER.ordinal()], uniformTransform, Semantic.Uniform.TRANSFORM0);

            gl3.glUseProgram(program[Program.SPLASH.ordinal()]);
    ...
    protected boolean render(GL gl) {
...

        {
            ...
            gl3.glBindFramebuffer(GL_FRAMEBUFFER, debug ? 0 : framebufferName[0]);
