...
public class Gl_300_fbo_multisample extends Test {
    ...
    private boolean initTexture(GL3 gl3) {
        ...
        try {
            Texture2d texture = (Texture2d) jgli.detail.LoadDds.loadDds(TEXTURE_ROOT + "/" + TEXTURE_DIFFUSE);
            jgli.Gl Gl = new Gl();
            ...
            jgli.Gl.Format format = Gl.translate(texture.format());

            for (int level = 0; level < texture.levels(); level++) {

                gl3.glTexImage2D(GL_TEXTURE_2D, level, format.internal.value,
                        texture.level(level).dimensions()[0], texture.level(level).dimensions()[1],
                        0, format.external.value, format.type.value, texture.level(level).data());
            }

            checkError(gl3, "initTexture");
    ...
    private boolean initFramebuffer(GL3 gl3) {
        ...
        gl3.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, FRAMEBUFFER_SIZE.x, FRAMEBUFFER_SIZE.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
